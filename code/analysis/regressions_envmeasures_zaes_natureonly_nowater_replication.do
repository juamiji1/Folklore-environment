...
PROJECT:
AUTHOR: JMJR
TOPIC: Final ZAES with multi-cluster SEs (single coef, 4 SE rows)
       — Outcomes: BII + Tree Cover (Hansen) only. Seasonal water DROPPED.
DATE:

NOTES: Mirrors regressions_envmeasures_zaes_natureonly_replication.do but
       restricts the AES outcome bundle to two measures:
         * bii            — Biodiversity Intactness Index
         * sh_treecover   — Tree Cover Share (Hansen)
       Seasonal surface water (sh_seasonwater_base) is excluded — this is the
       canonical "final" specification.
       ...
       Output: Table_folklore_zaes_natureonly_final.tex.
------------------------------------------------------------------------------*/
...
*    Outcomes restricted to {bii, sh_treecover} (no seasonal water).
*===============================================================================
...
gl depvars  "bii sh_treecover"
gl zdepvars "std_bii std_sh_treecover"
...
esttab a1 a2 a3 a4 a5 ///
	using "${tables}/Table_folklore_zaes_natureonly_nowater_final.tex", ///
...
di _n "ZAES final (BII + tree cover) replication completed!"
