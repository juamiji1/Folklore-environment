...
PROJECT:
AUTHOR: JMJR
TOPIC: Final per-outcome regressions — Hansen vs MODIS tree cover, positive only
       Multi-cluster SEs (single coef, 4 SE rows). Sample restricted to
       polygons with `yvar > 0`.
DATE:

NOTES: Mirrors the multi-cluster SE structure of
       regressions_envmeasures_peroutcome_natureonly_replication.do but the
       outcome set is {sh_treecover, sh_treecover_modis} and the sample is
       restricted to polygons with strictly positive tree-cover share for the
       outcome being estimated (matches the convention in
       regressions_envmeasures_peroutcome_natureonly_treecover_comparison.do).
       ...
       Output: one table per outcome — Table_peroutcome_<yvar>_final_comparison.tex.
------------------------------------------------------------------------------*/
...
*===============================================================================
* 1. Preparing everything for estimating the tree-cover comparison regressions
*
*===============================================================================
...
*===============================================================================
* 2. Per-outcome estimation — same regression with 4 clusterings stacked under each coef
*    Sample restricted to polygons with `yvar' > 0 (positive tree cover only).
*===============================================================================
...
foreach yvar of global depvars {
	...
	*-------------------------------------------------------------------------------
	...
	forval col = 1/5 {
		...
		* Run once per cluster on the positive-`yvar' subsample;
		...
		* reghdfe handles 1-way and 2-way clustering uniformly.
		...
		foreach cl of local clvars {
			local ++i
			qui reghdfe std_`yvar' ${X`c'} ${X1_int} ///
				if ${IF} & `yvar' > 0, noabsorb vce(cluster `cl') keepsing
	...
	*-------------------------------------------------------------------------------
	forval col = 1/5 {
		...
		foreach cl of local clvars {
			local ++i
			qui reghdfe std_`yvar' ${X`c'} ${X2_int} ///
				if ${IF} & `yvar' > 0, noabsorb vce(cluster `cl') keepsing
	...
	*-------------------------------------------------------------------------------
	esttab a1 a2 a3 a4 a5 ///
		using "${tables}/Table_peroutcome_`yvar'_final_comparison.tex", ///
	...
	*-------------------------------------------------------------------------------
	...
	di _n "Per-outcome (final, `yvar' > 0) table for `yvar' completed!"
}
