...
PROJECT: 
AUTHOR: JMJR
TOPIC: Regressions with Nature Exclusive Measures (REPLICATION) — ZAES with Conley SEs (500 km)
DATE:

NOTES: Mirrors regressions_envmeasures_aes_natureonly_NEW.do but the ZAES
       section uses Conley spatial standard errors (500 km cutoff, uniform
       kernel) via acreg, instead of one-way clustered SEs via reg.

       The AES (avg_effect) block is omitted because avg_effect does not
       expose a hook for spatial SEs. Only the ZAES outcome is reported here.
...
*===============================================================================
* Z-AES results — one outcome, 12 columns, Conley SEs (500 km cutoff)
*
*===============================================================================
...
forval c=1/6{
	...
	* Estimation of 1st spec — acreg with Conley spatial SEs (500 km cutoff)
	eststo zaes`c': acreg aes_z ${X`c'} ${X1_int} if ${IF}, ///
	...
	* Estimation of 2nd spec — acreg with Conley spatial SEs (500 km cutoff)
	eststo zaes`k': acreg aes_z ${X`c'} ${X2_int} if ${IF}, ///
		id(_conley_id) time(_conley_t) spatial latitude(lat) longitude(lon) dist(${DIST})
	gl n`k' = "`e(N)'"
...
esttab zaes1 zaes2 zaes3 zaes5 zaes6 zaes7 zaes8 zaes9 zaes11 zaes12 ///
	using "${tables}/Table_folklore_zaes_natureonly_conley500km.tex", ///
	...
	prehead(`"\begin{tabular}[t]{l*{10}{c}}"' ///
			`"\toprule"' ///
			`" & \multicolumn{10}{c}{Environmental Measures (ZAES)- Nature Exclusive (Conley SEs, 500 km)} \\"' ///
			`"\cmidrule(lr){2-11}"' ///
...
di _n "Z-AES with Conley SEs (500 km) — table completed!"
