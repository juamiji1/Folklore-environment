...
TOPIC: Per-outcome regressions with Nature Exclusive Measures (REPLICATION)
       — DFITS-based trimming of influential observations
DATE:

NOTES: Mirrors regressions_envmeasures_peroutcome_natureonly.do
       (one table per outcome, treatment alternates between X1_int and X2_int,
        12 columns from a single forval c=1/6 loop with k = c + 6)
       but applies a Belsley–Kuh–Welsch DFITS filter before reporting.

       For every column:
         (1) Fit plain OLS (no vce) to obtain leverage-based DFITS.
             [predict, dfits is not allowed after vce(cluster ...)/robust,
              so the diagnostic regression must be plain OLS.]
         (2) Cutoff = 2 * sqrt(k / n) where k = e(df_m) + 1 (params incl. constant)
             and n = e(N) — the standard Belsley–Kuh–Welsch threshold.
         (3) Re-fit the SAME specification with vce(cluster eafolk_id) on the
             trimmed sample (|dfits| <= cutoff). The trimmed regression is what
             gets stored via eststo and shown in the table.

       Outcomes are standardized within ${IF} so coefficients are on a z-score
       scale (matches the non-dfits per-outcome file). Standardization uses the
       full ${IF}-eligible sample, not the post-trim sample, so units stay
       stable across columns.

       Output filenames append `_dfits` so this can run alongside the
       non-trimmed version without overwriting.
...
la var changewater_abs     "Surface Water Change (abs)"
...
*   DFITS trimming applied per column.
*-------------------------------------------------------------------------------
...
foreach yvar of global depvars {
	...
	forval c=1/6 {
		...
		* predict, dfits is not allowed after vce(cluster ...) / vce(robust),
		* so fit plain OLS first, then re-fit with cluster SEs on the trimmed sample.
		qui reg std_`yvar' ${X`c'} ${X1_int} if ${IF}
		cap drop dfits_`c'
		...
		* Final estimation on the trimmed sample
		eststo c`c': reg std_`yvar' ${X`c'} ${X1_int} ///
		...
		* Diagnostic OLS (plain) — used only to compute dfits.
		qui reg std_`yvar' ${X`c'} ${X2_int} if ${IF}
		cap drop dfits_`k'
		...
		* Final estimation on the trimmed sample
		eststo c`k': reg std_`yvar' ${X`c'} ${X2_int} ///
	...
	*-------------------------------------------------------------------------------
	esttab c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 ///
		using "${tables}/Table_peroutcome_`yvar'_dfits.tex", ///
		keep(${X1_int} ${X2_int}) ///
		coeflabels( ///
		...
		prehead(`"\begin{tabular}[t]{l*{12}{c}}"' ///
				`"\toprule"' ///
				`" & \multicolumn{12}{c}{`ylab' - Nature Exclusive (DFITS-trimmed)} \\"' ///
				`"\cmidrule(lr){2-13}"' ///
...
di _n "Per-outcome DFITS-trimmed tables completed!"
