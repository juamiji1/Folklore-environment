...
PROJECT:
AUTHOR: JMJR
TOPIC: Per-outcome regressions with multi-cluster SEs (single coef, 4 SE rows)
DATE:

NOTES: Mirrors regressions_envmeasures_zaes_natureonly_replication.do but
       estimates per-outcome (one table per outcome) instead of pooling into a
       ZAES. Each coefficient is reported once and four standard errors are
       stacked below it — one per clustering scheme — with stars derived from
       each cluster's own p-value (not from the coefficient).
       ...
       Output: one table per outcome — Table_peroutcome_<yvar>_final.tex.
------------------------------------------------------------------------------*/
...
*===============================================================================
* 1. Preparing everything for estimating per-outcome regressions
*
*===============================================================================
...
foreach yvar of global depvars {
	...
	forval col = 1/5 {
		...
		* reghdfe handles 1-way and 2-way clustering uniformly.
		...
		foreach cl of local clvars {
			local ++i
			qui reghdfe std_`yvar' ${X`c'} ${X1_int} if ${IF}, noabsorb vce(cluster `cl') keepsing
	...
	*-------------------------------------------------------------------------------
	forval col = 1/5 {
		...
		foreach cl of local clvars {
			local ++i
			qui reghdfe std_`yvar' ${X`c'} ${X2_int} if ${IF}, noabsorb vce(cluster `cl') keepsing
	...
	*-------------------------------------------------------------------------------
	esttab a1 a2 a3 a4 a5 ///
		using "${tables}/Table_peroutcome_`yvar'_final.tex", ///
	...
	*-------------------------------------------------------------------------------
	...
	di _n "Per-outcome table for `yvar' completed!"
}
