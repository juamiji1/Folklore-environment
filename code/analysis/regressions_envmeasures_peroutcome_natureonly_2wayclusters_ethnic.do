...
TOPIC: Per-outcome regressions with Nature Exclusive Measures (REPLICATION)
       — 2-way clustered SEs (eafolk_id + country_code)
DATE:

NOTES: Mirrors regressions_envmeasures_peroutcome_natureonly_NEW.do but the
       estimation uses two-way clustering on (eafolk_id, country_code) via
       reghdfe instead of one-way (eafolk_id) via reg.

       Output filenames append `_2wcluster` so this can run alongside the
       1-way file without overwriting the parent's outputs.
...
foreach yvar of global depvars {
	...
	forval c=1/6 {
		...
		* Estimation — reghdfe with 2-way clustered SEs
	...
	*-------------------------------------------------------------------------------
	esttab c1 c2 c3 c5 c6 c7 c8 c9 c11 c12 ///
		using "${tables}/Table_peroutcome_`yvar'_2wcluster.tex", ///
		keep(${X1_int} ${X2_int}) ///
		coeflabels( ///
		...
		postfoot(`" & & & & & & & & & & \\"' ///
				 ...
				 `"Ethnic-folklore clusters & ${clf1} & ${clf2} & ${clf3} & ${clf5} & ${clf6} & ${clf7} & ${clf8} & ${clf9} & ${clf11} & ${clf12} \\"' ///
				 `"Country clusters & ${clc1} & ${clc2} & ${clc3} & ${clc5} & ${clc6} & ${clc7} & ${clc8} & ${clc9} & ${clc11} & ${clc12} \\"' ///
				 `"\bottomrule"' ///
				 `"\end{tabular}"')
}

di _n "Per-outcome tables (2-way clustered SEs) completed!"
