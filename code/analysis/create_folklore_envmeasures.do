...
import delimited "${maps}/raw\BII\ethnologue_bii_byyear.csv", clear

tempfile BIIY
save `BIIY', replace 

import delimited "${maps}/raw\Water_surface\ethnologue_seasonalwater_byyear_fixed.csv", clear

tempfile SEAWATERY
save `SEAWATERY', replace 

import delimited "${maps}/raw\Hansen_forest\yearloss\ethnologue_treeloss_byyear_fixed.csv", clear
...
merge 1:1 id using `BIIY', keep(1 3) nogen 
merge 1:1 id using `SEAWATERY', keep(1 3) nogen 
merge 1:1 id using `TREESY', keep(1 3) nogen
...
* Creating yearly variables 
gen treecover_km2_2000=treecover_km2
gen sh_treecover_2000=sh_treecover

forval y=2001/2024{	
	local k=`y'-1
	
	gen treecover_km2_`y' = treecover_km2_`k' - treeloss_y`y'_km2
	gen sh_treecover_`y'=treecover_km2_`y'*100/area_km2
	replace sh_treecover_`y'=100 if sh_treecover_`y'>100 & sh_treecover_`y'!=.
}

gen bii_2000=bii

forval y=2005(5)2020{
	replace bii_`y'=0 if bii_`y'==.
}

gen seasonalwater_y2000_km2=seasonalwater_2000_km2
gen sh_seasonwater_2000=sh_seasonwater_base

forval y=2001/2021{	
	local k=`y'-1
	
	gen sh_seasonwater_`y'=seasonalwater_y`y'_km2*100/area_km2
	replace sh_seasonwater_`y'=100 if sh_seasonwater_`y'>100 & sh_seasonwater_`y'!=.
}
